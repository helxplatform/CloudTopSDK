...
def ctbuild():
    ...
    if ("/" in tag) and (":" in tag):
        CLOUDTOP_HEADER_LINE = CLOUDTOP_HEADER_LINE.replace("GENERATOR", thisExecutable).replace("TIME", now).replace(
            "helxplatform/cloudtop:CLOUDTOP_VERSION", tag)
    else:
        CLOUDTOP_HEADER_LINE = CLOUDTOP_HEADER_LINE.replace("GENERATOR", thisExecutable).replace("TIME", now).replace(
            "CLOUDTOP_VERSION", tag)
